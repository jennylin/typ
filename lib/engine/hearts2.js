...
Game
  hasSpace/s([number,] name, holds...) // create space(s) root node and instance/collection reference as name. Optional holds sets restriction and singular/plural on the piece collection and creates reference to piece collection on this space
...
Player // player instance, has spaces, pieces, holds pieces through spaces
  hasSpace/s([number,] name, holds...) // create space(s) node as child on this player, and instance/collection reference as name. Optional holds sets restriction and singular/plural on the piece collection and creates reference to piece collection on this space
  hasPieces(singular, plural, names..., methods) // create pieces instances and create reference to collection with plural name
  hasPiece(name, methods) // create piece instance and create reference to instance with name
  spaces // collection
  pieces // collection
...
hearts
  .hasSpace('deck', { holds: 'cards', visible: false });

hearts.players
  .hasSpace('hand', { holds: 'cards' })
  .hasSpace('played', { holds: 'card' })
  .hasSpace('tricks', { holds: 'cards' })
  .cannotSee(function(me) {
    return [
      this.hands.except({ player: me }).cards,
      this.tricks.cards
    ];
  });
...
hearts.when('start play', function() {
  this.lead = this.players.one({ with: '2c' });
  this.become('lead');
});
...
hearts.when('end deal', function() {
  ...
  if (this.players.highest('points')==this.cards.sum('points')) {
    this.players.each(function(player) {
      player.points = player.points==0 ? this.cards.sum('points') : 0; // reverse the points if someone took all the points
    });
  }
  this.players.each(function(player) {

...
_.extend(Game.prototype, CanHavePieces, CanHaveSpaces, Item, {
  ...
  become: function(phase) {
    if (_.isFunction(this._phases[phase])) {
      ...
      this.game.refreshDom(this._currentPlayer);
  ...
  refreshDom: function(player) {
    ...
    if (this.client) {
      this.client.refresh(GameEngine.GameXML.get(this, player));
    }
  },
...
_.extend(Player.prototype, CanHaveSpaces, CanHavePieces, Item, {
  ...
  isCurrent: function() {
    if (this.game._currentPlayer!=this) {
      this.game.refreshDom(this);
    }
...
var GameXML = GameEngine.GameXML = {
  get: function(game, viewer) {
    ...
    game.players.each(function(player) {
      GameXML.addNode($('<player>', { class: 'player', name: player.name, jsonpath: player.jsonpath() }).appendTo(board), player, game, viewer);
    });

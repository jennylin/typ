...
var Collection = {
  ...
  _test: function(item, key, value) {
    ...
    if (key=='in') { // item is child, value is item or collection or array of items
      if (_.isArray(value)) {
        return _.any(value, function(v) { return v && v.contains && v.contains(item); });
      }
      if (_.isObject(value)) {
        if (value.contains) {
        ...
        if (value.all) { 
          return _.any(value.all(), function(v) { return v.contains(item); });
        }
      }
    ...
    if (key=='with') { // item is parent, value is item or where query
      ...
      return _.any(this.where.call(this.game.allPieces, value).all(), function(v) { return item.contains(v); });
    }
...
_.extend(Game.prototype, CanHavePieces, CanHaveSpaces, Item, {
  ...
  become: function(phase) {
    if (_.isFunction(this._phases[phase])) {
      ...
      this.game.refreshDom(this.player(1));
  ...
  // copy to dom - placeholder
  refreshDom: function(player) {
    if (!this._started) { return; }
    if (this.dom) {
      this.dom.html(new GameEngine.GameXML(this, player));
...
var Item = {
  ...
  _position: function() {
    ...
    return this.parent.allPieces.all().indexOf(this);
  }
};
...
_.extend(Move.prototype, {
  ...
  prompt: function(player, callback) {
    ...
    this.game.refreshDom(player);

    this.player = player;
    var pieces = this.pieceChoices.call(this.game, this);
    pieces = pieces._isCollection ? pieces.list('jsonpath') : _.invoke(pieces, 'jsonpath');
    ...
    this.game.ask(player, question, function(answer) {
      ...
      move.to = move.spaceChoices.call(move.game, move);
      if (move.to._isCollection || _.isArray(move.to)) {
        question = {
          type: 'select',
          description: move.description(),
          options: move.to._isCollection ? move.to.list('jsonpath') : _.invoke(move.to, 'jsonpath'),
          continuation: true
        };
...
var GameXML = GameEngine.GameXML = function(game, player) {
  ...
  this.game.players.each(function(player) {
    xml.addNode($('<player>', { class: 'player', name: player.name, jsonpath: player.jsonpath() }).appendTo(board) , player);
  });
...
_.extend(GameXML.prototype, {
  addNode: function(node, parent) {
    _.each(_.where(this.game._spaces, { parent: parent }), function(space) {
      if (!this.player || this.player._canSee(space)) {
        var $el = $('<' + inflection.underscore(space.name) + '>', { class: 'space', jsonpath: space.jsonpath() });
        _.each(space.attributes, function(attr) {
          $el.attr(attr, space.get(attr));
        });
    ...
    _.each(_.where(this.game._pieces, { parent: parent }), function(piece) {
      var $el = $('<' + inflection.underscore(piece.type) + '>', { class: 'piece', jsonpath: piece.jsonpath() });
      if (!this.player || this.player._canSee(piece)) {

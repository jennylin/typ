...
_.extend(Game.prototype, CanHavePieces, CanHaveSpaces, Item, {
  ...
  ask: function(player, question, callback) {
    if (!question.choices || question.choices.length==0) {
      throw('no choices for question ' + question.description);
    }
    if (!_.isArray(question.choices)) {
      return callback.call(this, { answer: [question.choices] }); // only one possible option - skip client ask
    }
    this._question++;
...
_.extend(Move.prototype, {
  ...
  prompt: function(player, callback) {
    ...
    this.player = player;
    var pieces = this._serialize(this.pieceChoices.call(this.game, this));
    if ((_.isArray(pieces) ? pieces.length : (pieces ? 1 : 0)) < this.minimum) {
    ...
    this.game.ask(player, question, function(answer) {
      var pieces = answer.answer;
      if (!pieces) {
        return false;
      }
      move.pieces = [];
      _.each(pieces, function(piece) {
        if (question.choices!=piece && !_.contains(question.choices, piece)) {
          throw('not an option: ' + piece);
        }
        move.pieces.push(move.game._deserialize(piece));
      });
      ...
      question = {
        ...
        choices: move._serialize(move.spaceChoices.call(move.game, move)),
        continuation: true
      };
      move.game.ask(player, question, function(answer) {
        move.to = move.game._deserialize(answer.answer[0]);
  ...
  _serialize: function(objects) {
    ...
    return objects._serialize(this.player);
  }
});

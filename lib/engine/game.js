...
Game
  hasSpace/s([number,] name, holds...) // create space(s) root node and instance/collection reference as name. Optional holds sets restriction and singular/plural on the piece collection or value and creates reference to piece collection on this space
...
var Collection = {
  ...
  items: function() {
    var collection = this;
    return _.filter(this.game._pieces, function(item, index, items) {
  ...
  _test: function(item, index, items, key, value) {
    ...
    if (key=='in') { return item.parent==value; } // should recurse
...
var Spaces = module.exports.Spaces = function() {
  return this._bindWhere();
};

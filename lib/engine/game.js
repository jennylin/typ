...
game owns players, spaces, pieces

player owns spaces, pieces

space owns spaces, holds pieces
...
var Collection = {
  ...
  _test: function(item, key, value) {
    ...
    if (key=='with') { return value.parent==item; } 
...
_.extend(Pieces.prototype, Collection, {
  move: function() {
    ...
    return {
      to: function(space) {
        var moves = pieces.where.apply(pieces, query);
        if (moves.each) {
          moves.each(function(piece) { piece.moveTo(space); });
        } else {
          moves.moveTo(space);
        }
      }
...
_.extend(Space.prototype, Item, {
  ...
  has: function() {
    return this.apply.some(this, arguments);
  }
});

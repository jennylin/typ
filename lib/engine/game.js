...
TODO:
 more error checking
 any cleanup
 recursion: spaces/pieces
 multi-space/piece children
 filter.owner remove?
 inflections?
...
CanHaveSpaces = {
   ...
   */
  hasSpace: function(singular, plural, holds) {
    this[singular] = this.createSpace(singular, plural, holds);
    return this;
  },

  hasSpaces: function(singular, plural, number, holds) {
    _(number).times(function() {
      this.createSpace(singular, plural, holds);
    }, this);
  ...
  createSpace: function(singular, plural, holds) {
    var space = new Space({ game: this.game, name: singular, owner: this }).holds(holds);
    this.game._spaces.push(space);

    // create collections on game node for nested spaces
    if (this!==this.game) {
      this.game[plural] = new Spaces({
        game: this.game,
        type: singular
      }).holds(holds);
      this.game[singular] = new Spaces({
        game: this.game,
        type: singular,
        singular: true
      }).holds(holds);
...
CanHavePieces = {
   ...
   */
  hasPiece: function(singular, plural, methods) {
    this[singular] = this.createPiece(singular, singular, methods);
    return this;
  },
...
var Collection = {
  ...
  _test: function(item, key, value) {
    ...
    if (key=='in') { // item is child, value is item or array of items
      return _.isArray(value) ? _.any(value, function(v) { return v.contains(item); }) : value.contains(item);
      /*
        (_.isArray(value) ? _.contains(value, item.parent) : item.parent==value) ||
        (item.parent && item.parent._test && item.parent!=this.game && this._test.call(item.parent, item.parent, 'in', value));
      */
    }
    if (key=='with') { // item is parent, value is item or where query
      ...
      return _.any(this.where.call(this.game.pieces, value).items(), function(v) { return item.contains(v); });
    }
    // return value.parent ? value.parent==item : item.some(value);

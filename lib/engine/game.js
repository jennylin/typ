...
Game._validateName = function(word) {
  ...
  if (_.contains('game', 'name', 'piece', 'space', 'player', 'pieces', 'spaces', 'players')) {
...
var Item = {
  ...
  _position: function() {
    ...
    return this.parent.pieces._items().indexOf(this);
  }
};
...
_.extend(Player.prototype, CanHaveSpaces, CanHavePieces, Item, {
  ...
  must: function(name, fn) {
    ...
    _.clone(move).prompt(this, function(move) {
...
_.extend(Move.prototype, {
  ...
  prompt: function(player, callback) {
    ...
    if (pieces.quantity() < this.minimum) {
      throw("not enough valid pieces to move for " + this.name);
    }

    question = {
      id: this.game._question,
      type: 'select',
      multiple: this.maximum!=1,
      description: this.description(),
      options: pieces.list('jsonpath')
    };
    ...
    this.game.ask(player, question, function(answer) {
      ...
      if (move.maximum==1) { move.piece = move.pieces[0]; }

  ...
  prompt: function(player, callback) {
    ...
    bootbox.prompt({
      ...
      callback: function(piece) {
        ...
        if (this.maximum==1) { this.piece = this.pieces.first(); }
        console.log(this.pieces.list());
        if (!this.pieces) {
          bootbox.alert(piece + " is not an option");
          return false;
        }
        if (this.minimum!==undefined && this.pieces.quantity() < this.minimum) {
          bootbox.alert(piece + " is not enough");
          return false;
        }
        if (this.maximum!==undefined && this.pieces.quantity() > this.maximum) {
          bootbox.alert(piece + " is too many");
          return false;
        }
        if (this.valid && !this.valid.call(this.game, this)) {
          bootbox.alert({
...
_.extend(GameXML.prototype, {
  addNode: function(node, parent) {
    _.each(_.where(this.game._spaces, { parent: parent }), function(space) {
      var $el = $('<' + inflection.underscore(space.name) + '>', { class: 'space' });
    ...
    _.each(_.where(this.game._pieces, { parent: parent }), function(piece) {
      var $el = $('<' + inflection.underscore(piece.type) + '>', { class: 'piece' });

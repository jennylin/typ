...
var Item = {
  ...
  others: function(fn) {
    ...
    if (fn) {
      return others.where(_.filter(others.all(), function(other) {
...
_.extend(Move.prototype, {
  ...
  prompt: function(player, callback) {
    ...
    this.game.ask(player, question, function(answer) {
      ...
      if (move.to._isCollection && move.to.quantity()==1) {
        move.to = move.to.first();
      } else if (_.isArray(move.to) && move.to.length==1) {
        move.to = move.to[0];
      }

...
describe("Q", function() {
  ...
  it("players have pieces and spaces too", function() {
    ...
    game
      ...
      .hasSpace('deck', 'decks', 'cards');

    game.players
      .haveSpace('hand', 'hands', 'cards')
      .havePieces('army', 'armies', 10)
      .havePiece('token', 'tokens');

    game.hasSpace('field', 'fields', 'armies');
    ...
    expect(game.player(1).armies({ first: 3 }).size()).toBe(3);
  });
  ...
  it("creates plural spaces", function() {
    ...
    expect(game.squares(1).tokens.size()).toBe(1);
    expect(game.squares(8).tokens.size()).toBe(1);
  });
  ...
  it("moves pieces", function() {
    ...
    expect(game.space({ with: '2C' })).toBe(game.turnUp);
    ...
    expect(function() { game.cards({ first:1 }).moveTo(game.turnUp); }).toThrow('turnUp can only hold one \'card\'');
  });

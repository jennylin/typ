...
var Player = exports.Player = function(game, userId, position) {
  this.game = game;
  this.userId = userId;
  this.position = position;
};
...
GameManager.prototype.joinGame = function(id, position, cb) {
  this.loadGame(id, function(err, game) {
    if (err) return cb(err);
    this.db('players').insert({game_id: id, position: position, user_id: this.userId}).exec(function(err, resp) {
      if (err) return cb(err);
      cb(null, game);
    });
  });
...
GameManager.prototype.addPlayers = function(game, cb) {
  var manager = this;
  manager.db('players').where('game_id', game.id).exec(function(err, players) {
    if (err) return cb(err);
    players.forEach(function(player) {
      game.players[player.position] = new Player(game, player.userId, player.position)
    });

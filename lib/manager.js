...
var GameManager = module.exports = function() {
  ...
  console.log("done game manager...");
}

GameManager.prototype.createUser = function(username, password, confirm, cb) {
  if (password !== confirm) {
    cb("Password and confirm must match");
  } else {
    this.db('users').insert({username: username, password: passwordHash.generate(password)}).exec(function(err, resp) {
      if (err) {
        return cb(err);
      }

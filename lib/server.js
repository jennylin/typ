...
function authenticate(req, res, next) {
  if (req.query._id) {
    req.session.expiresAt = new Date().getTime() + 1000000;
    req.session.userId = req.query._id;
  }
...
var HttpServer = exports.HttpServer = function() {
  ...
  this.app.post('/games/:game_id/moves', authenticate, function(req, res) {
    console.log("getting a move....");
    req.gameManager.loadGame(req.params.game_id, function(err, game) {
      console.log("game loaded");
      if (err) return res.send(err.toString());
      if (game.answerQuestion(parseInt(req.body.id), JSON.parse(req.body.answer))) {
        console.log("question answered");
        req.gameManager.saveGame(game, function(err) {

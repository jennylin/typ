...
var HttpServer = exports.HttpServer = function() {
  ...
  this.app.post('/login', function(req, res) {
    req.gameManager.validateUser(this.body.name, this.body.password, function(err, resp) {
      if (err) return res.send(500, err.toString());
      if (resp) {
  ...
  this.app.get('/games', authenticate, function(req, res) {
    req.manager.list(function(err, list) {
      if (err) return res.send(500, err.toString());
  ...
  this.app.get('/games/:game_id', authenticate, function(req, res) {
    manager.load(req.params.game_id, function(err, game) {
      if (err) return rest.send(err.toString());

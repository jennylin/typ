...
hearts.when('win trick', function() {
  var winner = this.player({
    with: this.playingAreas.cards({ suit: 'h' }).highest('value')
      || this.playingAreas.cards({ suit: this.led.suit() }).highest('value')
  });

  winner.numberOfTricks++;
  
  this.playingAreas.cards.moveTo(winner.tricks);

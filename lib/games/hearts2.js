...
hearts.hasMoves({
  ...
  passCards: {
    description: function() { return 'Pass 3 cards ' + [null, 'to your left', 'across', 'to your right'][this.round % 4]; },
...
hearts.when('play card', function() {
  this.currentPlayer.must('play', function(move) {
    ...
    if (this.playingAreas.cards.quantity()==4) {
      ...
      console.log('play card', this.currentPlayer);
      this.become('play card');
    }
  });
});

  ...
  .players
    .havePieces('pieces', {
      ...
      availableSquares: function(pushed) {
        return this.square && _.filter(this.square.adjacent(), function(square) {
          return square && square.none() && (pushed || this.name!='rabbit' || square!=this.backwards());
        }, this);
      }
    })
    ...
    move: {
      ...
      valid: function(move) {
        if (move.piece.player!=move.player && (!this.lastMove || !this.lastMove.isPull)) { // is pushing
          move.isPush = true;
          move.pushing = move.piece.strongerAdjacent();
        }
        if (move.piece.weakerAdjacent().some() && !this.lastMove.isPush) { // may be pulling
          move.isPull = true;

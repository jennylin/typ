...
function Event(type, body, recipient) {
  this.type = type;
  this.body = body;
...
Game.prototype.runCurrentState = function() {
  ...
  console.log("this.states[this.state]:"+JSON.stringify(this.states))
...
Game.prototype.transition = function(newState) {
  ...
  this.send('information', {name: "stateChange", from: this.state, to: newState});
  this.state = newState;
